...
# files = ['dr3211_1_filtered.csv']
# maneuver  = [(19.5, 29)]  # marking the timestamps in each csv files that contain the test manoeuvre, minimize fitting for wrong data

# files = ['da3211_2_filtered.csv']
# maneuver  = [(19, 40)]  # marking the timestamps in each csv files that contain the test manoeuvre, minimize fitting for wrong data
...
for i, filename in enumerate(files):
    ...
    if filename=='de3211_1_filtered.csv':
        CX_model = model(np.array([consts, betas_k, betas_k**2, alphas_k, alphas_k**2, alphas_k**3, qs_k*c/Vs_k, (qs_k*c/Vs_k)**2, de]).T,    name="CX model bigger")
        ...
        CZ_model = model(np.array([consts, alphas_k, alphas_k**2, qs_k*c/Vs_k, (qs_k*c/Vs_k)**2, de]).T,                                      name="CZ model bigger")
        CZ_model = model(np.array([consts, alphas_k, qs_k*c/Vs_k, de, Tc]).T,                 name="CZ model")
        CZ_model.measurements = FCs_k[2].reshape(-1,1)
            
        Cm_model = model(np.array([consts, alphas_k, alphas_k**2, qs_k*c/Vs_k, (qs_k*c/Vs_k)**2, de]).T,                                      name="Cm model bigger")
        ...
        Cl_model = model(np.array([consts, v, ps_k*b/2/Vs_k, (ps_k*b/2/Vs_k)**3, rs_k*b/2/Vs_k, da]).T, name="Cl model bigger")
        ...
        CY_model = model(np.array([consts, betas_k, v, ps_k*b/2/Vs_k, rs_k*b/2/Vs_k, dr]).T, name="CY model bigger")
        ...
        Cn_model = model(np.array([consts, v**3, betas_k, rs_k*b/2/Vs_k, dr]).T, name="Cn model bigger")
...
# files = ['dr3211_2_filtered.csv']
# maneuver  = [(19.5, 29)]  # marking the timestamps in each csv files that contain the test manoeuvre, minimize fitting for wrong data

# files = ['dadoublet_1_filtered.csv']
# maneuver  = [(10, 15)]  # marking the timestamps in each csv files that contain the test manoeuvre, minimize fitting for wrong data
...
for i, filename in enumerate(files):
    ...
    if filename=='dedoublet_1_filtered.csv':
        CX_model.regression_matrix = np.array([consts, betas_k, betas_k**2, alphas_k, alphas_k**2, alphas_k**3, qs_k*c/Vs_k, (qs_k*c/Vs_k)**2, de]).T
        CX_model.regression_matrix = np.array([consts, alphas_k, alphas_k**2, qs_k*c/Vs_k, de, Tc]).T
        CX_model.measurements      = FCs_k[0].reshape(-1,1)

        CZ_model.regression_matrix = np.array([consts, alphas_k, alphas_k**2, qs_k*c/Vs_k, (qs_k*c/Vs_k)**2, de]).T
        CZ_model.regression_matrix = np.array([consts, alphas_k, qs_k*c/Vs_k, de, Tc]).T
        CZ_model.measurements      = FCs_k[2].reshape(-1,1)
            
        Cm_model.regression_matrix = np.array([consts, alphas_k, alphas_k**2, qs_k*c/Vs_k, (qs_k*c/Vs_k)**2, de]).T
        Cm_model.regression_matrix = np.array([consts, alphas_k, qs_k*c/Vs_k, de, Tc]).T
        Cm_model.measurements      = MCs_k[1].reshape(-1,1)

        models                     = [CX_model, CZ_model, Cm_model]

    elif filename=='dadoublet_1_filtered.csv':
        Cl_model.regression_matrix = np.array([consts, v, ps_k*b/2/Vs_k, (ps_k*b/2/Vs_k)**3, rs_k*b/2/Vs_k, da]).T
        Cl_model.regression_matrix = np.array([consts, betas_k, ps_k*b/2/Vs_k, rs_k*b/2/Vs_k, da, dr]).T
        Cl_model.measurements      = MCs_k[0].reshape(-1,1)

        models                     = [Cl_model]

    elif filename=='dr3211_2_filtered.csv':
        CY_model.regression_matrix = np.array([consts, betas_k, v, ps_k*b/2/Vs_k, rs_k*b/2/Vs_k, dr]).T
        CY_model.regression_matrix = np.array([consts, betas_k, ps_k*b/2/Vs_k, rs_k*b/2/Vs_k, da, dr]).T
        CY_model.measurements      = FCs_k[1].reshape(-1,1)
        
        Cn_model.regression_matrix = np.array([consts, v**3, betas_k, rs_k*b/2/Vs_k, dr]).T
        Cn_model.regression_matrix = np.array([consts, betas_k, ps_k*b/2/Vs_k, rs_k*b/2/Vs_k, da, dr]).T
        Cn_model.measurements      = MCs_k[2].reshape(-1,1)
