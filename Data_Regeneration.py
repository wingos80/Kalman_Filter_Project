...
for filename in files:
    ...
    if filename.endswith(".csv") and not (filename.endswith("_measurements.csv") or filename.endswith("all_results.csv")):
        ...
        if save:
            ...
            result_file  = open(filename_new, "w")
        ...
        ########################################################################
        filename = 'figs/raw_and_noise_figs/' + filename.split('.')[0]
        ...
        if save:
            plt.savefig(f'{filename}_true_vs_noise_xyz.pdf')
        ...
        make_plots(x, [ys], f'{filename} true vs noise angles', r'Time $[s]$', [r'angles $[rad]$'], save=save, colors=colors)
        ...
        make_plots(x, [ys], f'{filename} true vs noise velocities', r'Time $[s]$', [r'velocities $[m/s]$'], save=save, colors=colors)
        ...
        make_plots(x, [ys], f'{filename} true vs noise accelerations', r'Time $[s]$', [r'accelerations $[m/s^2]$'], save=save, colors=colors)
        ...
        make_plots(x, [ys], f'{filename} true vs noise angular velocities', r'Time $[s]$', [r'angular velocities $[rad/s]$'], save=save, colors=colors)

        ys = {r'$V_{TAS}$ true': [vtas, alpha_t],
                r'$V_{TAS}$ noise': [airdata_t[0,:], alpha_n]}
        make_plots(x, [ys], f'{filename} true vs noise Vtas', r'Time $[s]$', [r'$V_{tas} [rad]$'], save=save, colors=colors)

        ys = {r'$\alpha$ true': [alpha, alpha_t],
                r'$\alpha$ noise': [airdata_t[1,:], alpha_n],
                r'$\beta$ true': [beta, alpha_t],
                r'$\beta$ noise': [airdata_t[2,:], alpha_n]}
        make_plots(x, [ys], f'{filename} true vs noise alpha and beta', r'Time $[s]$', [r'angles [rad]'], save=save, colors=colors)
        ...
        make_plots(x, [ys], f'{filename} true vs noise positions', r'Time $[s]$', [r'positions $[m]$'], save=save, colors=colors)
        if show_plot:
            plt.show()
